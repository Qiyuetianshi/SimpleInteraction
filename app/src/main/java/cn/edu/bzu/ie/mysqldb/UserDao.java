...
public class UserDao extends DbOpenHelper {
    ...
    //查询所有的romm
    public List<room> getAllroom(String patname){
        List<room> list = new ArrayList<>();
        try{
            getConnection();   // 取得连接信息
            String sql = "select * from room where patname=? order by ? desc";
            ...
            while(rs.next()){   // 这里原来用的是if，查询数据集合时应使用while
                ...
                item.setIsreadpat(rs.getInt("isreadpat"));
    ...
    public Userinfo getUserByUnameAndUpass(String uname, String upass){
        ...
        try{
            getConnection();   // 取得连接信息
            String sql = "select * from patinfo where uname=? and upass=?";
            pStmt = conn.prepareStatement(sql);
    ...
    public boolean addUser(Userinfo item){
        try{
            getConnection();   // 取得连接信息
            String sql = "select * from patinfo where uname=?";
            pStmt = conn.prepareStatement(sql);
            ...
            String sql2 = "insert into patinfo(uname, upass) values(?, ?)";
    ...
    public boolean updateroomstatepro(int room){
        try{
            getConnection();   // 取得连接信息
    ...
    public boolean updateroompat(int room){
        try{
            getConnection();   // 取得连接信息
